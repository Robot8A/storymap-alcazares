  ...
  function initMap(options, chapters) {
    ...
    legend.onAdd = function (map) {     
      ...
      for (const [key, value] of Object.entries(grades)) {
        var icon = iconStyle(value).createIcon();
        div.innerHTML += '<div class="legend-icon">' + icon.outerHTML + '</div><div class="legend-icon">' + key + '</div></br>';
      }
      return div;
    };      
    legend.addTo(map);
    ...
    for (i in chapters) {
      ...
      if ( !isNaN(parseFloat(c['Latitude'])) && !isNaN(parseFloat(c['Longitude']))) {
        ...
        markers.push(
          L.marker([lat, lon], {
            icon: iconStyle(c['Marker Color'] || 'blue'),
